...
for i in range(100):
	data = """messages[user_ids][100000176732853][offset]={0}
		&messages[user_ids][100000176732853][limit]=100000
		&messages[user_ids][100000176732853][timestamp]
		&client=mercury&__user=100008183341012&__a=1
		...
		&fb_dtsg=AQFbG2C9c977%3AAQEmJLBD7pEk&ttstamp=265817098715067579957555558658169109747666685511269107""".format(i * 100000)
	dat = c.post("https://www.facebook.com/ajax/mercury/thread_info.php?dpr=1", headers = headers, data = data)
	f=open("data\\" + str(i) + ".html", "w+")
	f.write(dat.content)
	f.close()
	break